...
describe("Tokenomics test", function () {
  ...
  it("Test without liquidity providing or invi staking", async () => {
    ...
    const swapInviToNative = async (swapAmountRatio: number) => {
    ...
    // unstake lp for lps
    tx = await lpPoolContract.connect(LP1).unstake(lpAmount);
    await tx.wait();
    tx = await lpPoolContract.connect(LP2).unstake(lpAmount);
    await tx.wait();
