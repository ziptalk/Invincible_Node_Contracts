...
contract EvmosInviCore is Initializable, OwnableUpgradeable {
    ...
    EvmosLiquidityProviderPool public lpPoolContract;
    EvmosInviTokenStake public inviTokenStakeContract;
    IEvmosLiquidStaking public evmosLiquidStaking;
    ...
    function initialize(address _stTokenAddr, address _evmosLiquidStakingAddr) initializer public {
        ...
        evmosLiquidStaking = IEvmosLiquidStaking(_evmosLiquidStakingAddr);
    ...
    function setLpPoolContract(address _lpPoolAddr) external onlyOwner {
        lpPoolContract = EvmosLiquidityProviderPool(_lpPoolAddr);
    }

    function setInviTokenStakeContract(address _inviTokenStakeAddr) external onlyOwner {
        inviTokenStakeContract = EvmosInviTokenStake(_inviTokenStakeAddr);
    }
    ...
    function stake(uint _principal, uint _leverageRatio, uint _lockPeriod, uint _slippage) external payable{
        ...
        // stake using evmosLiquidStaking
        evmosLiquidStaking.stake{value : _stakeInfo.principal}();
    ...
    function distributeStTokenReward() external onlyOwner {
        ...
        // request unstake to evmosLiquidStaking
        evmosLiquidStaking.createUnstakeRequest(nftReward + lpReward + inviStakerReward);
    ...
    function stakeLp() external onlyLpPool payable {
        // stake 
        evmosLiquidStaking.stake{value : msg.value}();
        emit Stake(msg.value);
    }

    function unstakeLp(uint _requestAmount) external onlyLpPool {
        // create unstake request
        evmosLiquidStaking.createUnstakeRequest(_requestAmount);
