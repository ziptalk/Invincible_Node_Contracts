...
const main = async () => {
  ...
  // set inviToken init condition
  const tx2 = await inviTokenContract.connect(deployer).setLendingPoolAddress(lendingPoolContract.address, { nonce: nonce + 1 });
  await tx2.wait();
  console.log("inviToken init condition set at " + nonce);

  // set ISPTToken init condition
  const tx3 = await iSPTTokenContract.connect(deployer).setInviSwapPool(inviSwapPoolContract.address, { nonce: nonce + 2 });
  await tx3.wait();
